"""
``telegram_configs`` migration verification (PostgreSQL + Alembic head).

Checks table presence, columns, uniqueness on ``bot_id``, and FK cascade rules.
"""
...
def _alembic_for_telegram_migration_tests() -> None:
...
def test_telegram_migration_applies_to_script_head(live_db_url: str, monkeypatch: pytest.MonkeyPatch) -> None:
    async def body() -> None:
        cfg = Config(str(PROJECT_ROOT / "alembic.ini"))
...
def test_telegram_configs_table_columns_uniques_and_fk_cascade(live_db_url: str) -> None:
    async def body() -> None:
        engine = create_async_engine(normalize_database_url(live_db_url), pool_pre_ping=True)
        try:
            async with engine.connect() as conn:
                ...
                assert str(nullable_tok) == "YES"

                uq = await conn.execute(
                    text(
                        """
                        SELECT 1 FROM pg_constraint
                        WHERE conname = 'uq_telegram_configs_bot_id'
                          AND contype = 'u'
                        """
                    )
                )
                    ...
                    await conn.execute(
                        text(
                            ...
                            WHERE tc.table_schema = 'public'
                              AND tc.table_name = 'telegram_configs'
